...
template<typename T, typename U> bool ckmin(T&& a,U&& b) { return b < a ? a = b, 1 : 0; }
template<typename T,typename U> bool ckmax(T&& a,U&& b) { return a < b ? a = b, 1 : 0; }
...
void solve(){
  ...
  forn(i,11){
    reads.pb(async(launch::async,&readerWriter::get,&rw,i));
    threads.eb(&readerWriter::put,&rw,2*i);
    
  }
  forn(i,sz(threads)){
    threads[i].join();
  }
  for(future<int> &f:reads){
    int x = f.get();
    cout << x << " ";
  }
  en;
